     ...
     value_t plist_form_process_info(int proc_id,int parent_id)
     {
          value_t t;
          t.alive = true;
          t.parent_alive = true;
     ...
     value_t* plist_find(process_list*list, plist_key_t element_id)
     {
          if(list == NULL)
               return NULL;
     ...
     plist_key_t plist_insert(process_list* list, value_t v)
     {    
          if(list->next == NULL)
          {
     ...
     {    
          ...
          while(current && current->element.free)
          {
