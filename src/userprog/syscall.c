...
syscall_handler (struct intr_frame *f)
{
   ...
   case SYS_OPEN:
   {
      ...
      //TODO: verrify pinter
      if (!pagedir_verify_pointer(file) || !pagedir_verify_variable_length(file))
      {
         process_exit(-1);
         thread_exit ();
      }
   ...
   case SYS_EXEC:
   {
      ...
      if (!pagedir_verify_pointer(buffer) || !pagedir_verify_variable_length(buffer))
      {
         process_exit(-1);
         thread_exit ();
      }
